      ...
      desired_versions: &mut DesiredVersions,
        ...
        */
        ...
        let command_formatted = format!(
          "sudo -n apt update -y && sudo -n apt install -y curl apt-transport-https &&\
          curl -fsSL https://pkgs.k8s.io/core:/stable:/v{}/deb/Release.key | sudo gpg --yes --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg &&\
